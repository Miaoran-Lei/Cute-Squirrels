...
def map(request):
    list_for_exhibit = []
    squirrels100 = random.sample(list(chart.objects.all()), 100)
    for sighting in squirrels100:
        list_for_exhibit.append({'x_axis':sighting.x_axis,'y_axis':sighting.y_axis})
    context = {'sightings': list_for_exhibit}
...
def mainmenu(request):
    squirrels = chart.objects.filter(have_image = True)
...
def add(request):
    ...
    #load things from web
        test1 = chart.objects.create(
        x_axis = request.POST.get('latitude',0),
        y_axis = request.POST.get('longtitude',0),
...
def edit(request,squirrel_id):
    test1 = chart.objects.get(pk = squirrel_id)
    if request.method == "POST":
        test1.x_axis = request.POST.get('latitude',0)
        test1.y_axis = request.POST.get('longtitude',0)
...
def search(request):
    ...
    try:
        squirrels = chart.objects.get(pk = squirrel_id)
    except:
...
def sightings(request):
    squirrels = chart.objects.order_by('Unique_Squirrel_ID')
    context = {'squirrels':squirrels,}
    return render(request,'htmls/sightings.html',context)

def stats(request):
    sights = chart.objects.all()
    # shift
    ...
    # Actvities
    foraging_n = sights.filter(foraging = True).count()
    eating_n = sights.filter(eating = True).count()
    climbing_n = sights.filter(climbing = True).count()
    chasing_n = sights.filter(chasing = True).count()
    running_n = sights.filter(running = True).count()
    total_squirrel = sights.filter(is_foraging = True).count() + sights.filter(is_foraging = False).count()
    foraging_pct = "{:.2%}".format(foraging_n / total_squirrel)
    eating_pct = "{:.2%}".format(eating_n / total_squirrel)
    chasing_pct = "{:.2%}".format(chasing_n / total_squirrel)
    running_pct = "{:.2%}".format(running_n / total_squirrel)
    climbing_pct = "{:.2%}".format(climbing_n / total_squirrel)
